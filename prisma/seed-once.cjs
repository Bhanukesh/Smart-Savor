// One-time production seed — plain CommonJS, no tsx/TypeScript needed, so it
// runs with only what's already in the standalone runtime image (@prisma/client).
// Identical data/logic to prisma/seed.ts. Wired into the Dockerfile CMD for
// exactly one deploy, then removed — see that commit's message for why.
