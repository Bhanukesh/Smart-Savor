...
export async function PATCH(req: Request, { params }: { params: { itemId: string } }) {
  if (!isUuid(params.itemId)) return NextResponse.json({ error: "invalid item id" }, { status: 400 });
  const parsed = patchBody.safeParse(await req.json().catch(() => null));
  if (!parsed.success) {
    return NextResponse.json({ error: parsed.error.flatten() }, { status: 400 });
  }
  const item = await updateApprovedListItem(params.itemId, parsed.data.action, parsed.data.note);
