...
> **Onboarding note (updated 2026-08-11).** Patients are **invite-code gated, then real Google sign-in via a dedicated patient Clerk app** — a dietitian creates the patient record (optionally with an email, which auto-sends the invite via Clerk's own invitation email — no SMS provider involved) and issues a one-time code; the patient enters it first, and only a valid, unredeemed code unlocks sign-up via Google. That patient Clerk app is deliberately separate from the dietitian one and left unrestricted (see CLAUDE.md for the exact reasoning) — the invite code remains the real gate either way. A phone/OTP path was tried and dropped (US carrier A2P 10DLC registration friction). There is **no public patient self-signup path** — that "regular use" self-serve option is scrapped in favor of invite-only; sign-up never runs without a valid code first. The mobile app's own signup screen is still on the older unverified phone-only path pending dedicated Expo work. Dietitians go through their own **Clerk** app, Google/Microsoft sign-in only, restricted so only invited emails can even create an account — a dietitian invites a colleague by email from `/team`, no self-serve dietitian signup either. See `er-design.md` §Part 1, Decisions 2–3.
