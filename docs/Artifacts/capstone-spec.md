...
> **Framing note (deliberate).** A **licensed practitioner sets the targets; agents operationalize them.** Agents **draft**, the dietitian **ratifies** — clinical authority stays human, so Smart Savor is a workflow tool, not a diagnostic one. Comorbidity safety is **hybrid and human-owned**: hard rules auto-exclude, AI flags uncertain interactions, the dietitian has the last word. **Receipts prove purchase; logs prove consumption** — Smart Savor reconciles the two and nudges only on the bought-but-not-logged gap. Health data is handled with security-conscious design (encryption + split auth: two separate Clerk applications, one per role, patients' feeding a custom session); no HIPAA-compliance claim is made.

> **Onboarding note (updated 2026-08-14).** Patients are **invite-code gated, then real Google sign-in via a dedicated patient Clerk app** — a dietitian creates the patient record (optionally with an email, which auto-sends the invite via Clerk's own invitation email — no SMS provider involved) and issues a one-time code; the patient enters it first, and only a valid, unredeemed code unlocks sign-up via Google. That patient Clerk app is deliberately separate from the dietitian one and left unrestricted (see CLAUDE.md for the exact reasoning) — the invite code remains the real gate either way. A phone/OTP path was tried and dropped (US carrier A2P 10DLC registration friction). There is **no public patient self-signup path** — that "regular use" self-serve option is scrapped in favor of invite-only; sign-up never runs without a valid code first. Since an invite code is single-use, a **returning-patient sign-in** (same Google identity, no code) covers logging back in after a logout on either platform — it looks the caller up against an existing account instead of redeeming a new one, so it's not a second signup path. `mobile/` mirrors web's Google sign-in (and this returning-sign-in) exactly, not a separate phone-based path. Dietitians go through their own **Clerk** app, Google/Microsoft sign-in only, restricted so only invited emails can even create an account — a dietitian invites a colleague by email from `/team`, no self-serve dietitian signup either. See `er-design.md` §Part 1, Decisions 2–3.
